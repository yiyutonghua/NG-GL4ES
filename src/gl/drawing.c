...
void gl4es_glMultiDrawElementsBaseVertex(GLenum mode, GLsizei *counts, GLenum type, const void * const *indices, GLsizei primcount, const GLint * basevertex) {
    ...
    if (counts == NULL || indices == NULL || basevertex == NULL || primcount <= 0) {
        errorShim(GL_INVALID_VALUE);
        return;
    }

    for (GLsizei i = 0; i < primcount; i++) {
        GLsizei count = counts[i];
        GLenum elementType = type;
        const void *indexData = indices[i];
        GLint baseVert = basevertex[i];
        ...
        gl4es_glDrawElementsBaseVertex(mode, count, elementType, indexData, baseVert);
    }
}
void gl4es_glMultiDrawElementsBaseVertex_bak( GLenum mode, GLsizei *counts, GLenum type, const void * const *indices, GLsizei primcount, const GLint * basevertex) {
    DBG(printf("glMultiDrawElementsBaseVertex(%s, %p, %s, @%p, %d, @%p), inlist=%i, pending=%d\n", PrintEnum(mode), counts, PrintEnum(type), indices, primcount, basevertex, (glstate->list.active)?1:0, glstate->list.pending);)
...
//            iindices = copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data + (uintptr_t)indices[i]):indices[i],
//                type, 1, 0, GL_UNSIGNED_INT, 1, 0, count, NULL);
//        else
//            sindices = copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data + (uintptr_t)indices[i]):indices[i],
//                type, 1, 0, GL_UNSIGNED_SHORT, 1, 0, count, NULL);
//
//        if (compiling) {
//            // TODO, handle uint indices
...
//
//            normalize_indices_us(sindices, &max, &min, count);
//            clist = arrays_to_renderlist(clist, mode, min + basevertex[i], max + basevertex[i] + 1);
//            clist->indices = sindices;
//            clist->ilen = count;
//            clist->indice_cap = count;
//            //end_renderlist(list);
//
//            if(glstate->list.pending) {
//                NewStage(glstate->list.active, STAGE_POSTDRAW);
...
//        } else {
//            if(iindices)
//                for(int j=0; j<count; j++) iindices[j]+=basevertex[i];
//            else
//                for(int j=0; j<count; j++) sindices[j]+=basevertex[i];
...
//    }
    ...
    DBG(printf("gles_glDrawElementsBaseVertex loop x%d\n", primcount);)
    for (int i = 0; i < primcount; i++) {
    if (counts[i] > 0)
    //            gl4es_glDrawElementsBaseVertex(mode,
    //                                           counts[i],
    //                                           type,
    //                                           indices[i],
    //                                           basevertex[i]);
