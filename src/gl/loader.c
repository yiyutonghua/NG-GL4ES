...
#endif
#else
__declspec(dllimport) struct HINSTANCE__* __stdcall LoadLibraryW(const wchar_t*);
#endif
#include "logs.h"
#include "init.h"
#include "envvars.h"
...
#ifndef _WIN32
#ifndef NO_GBM
static const char* drm_lib[] = {"libdrm", NULL};
static const char* gbm_lib[] = {"libgbm", NULL};
#endif
...
void* open_lib(const char** names, const char* override) {
    ...
    if (totest) {
        ...
        if (p && strstr(p, "libasan.so")) sanitizer = 1;
    }
    // note: breaks address sanitizer
    if (!sanitizer && globals4es.deepbind) flags |= RTLD_DEEPBIND;
#endif
    if (override) {
        if ((lib = dlopen(override, flags))) {
            strncpy(path_name, override, PATH_MAX);
            if (!globals4es.nobanner) LOGD("LIBGL:loaded %s", path_name);
            return lib;
        } else {
        ...
        for (int i = 0; names[i]; i++) {
            for (int e = 0; lib_ext[e]; e++) {
                ...
                if ((lib = dlopen(path_name, flags))) {
                    if (!globals4es.nobanner) LOGD("loaded %s", path_name);
                    return lib;
                }
            }
        }
...
void* APIENTRY_GL4ES proc_address(void* lib, const char* name) {
    if (gles_getProcAddress) return gles_getProcAddress(name);
#ifdef AMIGAOS4
